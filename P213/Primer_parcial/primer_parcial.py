...
def listar_heroes(lista: list):
    """Funcion que recibe una lista de heroes y los muestra de manera amena para el usuario."""
...
def agregar_heroe():
    """Funcion que agrega un heroe a la lista de heroes, pidiendo al usuario que ingrese las características del heroe, y validando que los datos ingresados sean correctos."""
    heroe_nuevo = []
    ...
    while empresa_nueva != "DC Comics" and empresa_nueva != "Marvel Comics":
    ...
    while genero_nuevo != "M" and genero_nuevo != "F" and genero_nuevo != "NB":
        print("El genero debe ser 'M', 'F' o 'NB'. Intente nuevamente.")
        genero_nuevo = input("Ingrese el genero del heroe (M, F, NB): ")
    
    heroe_nuevo.append(genero_nuevo)

    color_ojos_nuevo = input("Ingrese el color de ojos del heroe (Azules, Verdes, Marrones, Negros): ")
    while color_ojos_nuevo != "Azules" and color_ojos_nuevo != "Verdes" and color_ojos_nuevo != "Marrones" and color_ojos_nuevo != "Negros":
    ...
    color_pelo_nuevo = input("Ingrese el color de pelo del heroe (Rubio, Castaño, Negro, Pelirrojo): ")
    while color_pelo_nuevo != "Rubio" and color_pelo_nuevo != "Castaño" and color_pelo_nuevo != "Negro" and color_pelo_nuevo != "Pelirrojo":
    ...
    while inteligencia_nueva != "low" and inteligencia_nueva != "average" and inteligencia_nueva != "good" and inteligencia_nueva != "high" and inteligencia_nueva != "genius":
...
def eliminar_heroe():
    """Funcion que elimina un heroe de la lista de heroes, pidiendo al usuario que ingrese el nombre del heroe a eliminar, y validando que el heroe exista en la lista."""
...
def ordenar_heroes_por_nombre():
    """Funcion que ordena la lista de heroes por nombre utilizando el método de ordenamiento burbuja, y devuelve la lista ordenada."""
...
def filtrar(dato: str, mayor_o_menor: str):
    """Funcion que filtra la lista de heroes por altura, fuerza o peso, dependiendo del valor del parámetro dato, y dependiendo del valor del parámetro mayor_o_menor, muestra el héroe con el valor más alto o más bajo de ese dato."""
    lista_filtrada = []
    maximo = 0
    minimo = 9999
    for heroe in lista_heroes:
